...
We see in Figure \ref{fig:model1} the equilibrium point of $(6 \frac{2}{3}, 3 \frac{1}{3})$. The orange line is the demand line that we are assuming in equation \eqref{eq:qd1}. The green line is the supply line that we assumed in equation \eqref{eq:qs1}. Consumer views on the product are assumed to not change over time, and so they are both held constant. The demand line in orange can also be thought of as the maximum quantity sold at any given price, and so only the values in the bottom left half of the plot should be analyzed as the top right is theoretically impossible.
In the plot below price is on the y-axis and quantity is the x-axis. It is almost the 90-degree rotation of the plot above. In neither of these plots is it clear what equilibrium is without the original linear system also plotted. 
